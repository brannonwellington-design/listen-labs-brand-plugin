...
**Required parts:** branded header · stage columns (typically 4–7: e.g., Trigger → Research → Compare → Decide → Onboard) · swimlane rows, top to bottom: **Actions** (what they do), **Touchpoints** (where), **Thinking** (verbatims), **Emotion curve**, **Coded emotion** (the Ekman code and its base per stage — “Anger · 4 of 12” — because a curve cannot carry a count honestly; “Not coded” where none was recorded), **Opportunities** (optional bottom lane) · a base note (n=, method).

**Skeleton:** Horizontal layout; stages are equal-width columns separated by 1px hairlines in `--content-disabled`. Stage names at the top of each column, 14–16px. Lane labels pinned left in `--content-secondary`, 12px, Title Case. The **emotion curve** is the visual heart: a single SVG path spanning all stages, y = emotional valence, drawn at 2px because it is the story line (everything else stays 1px). Each measured point gets a 6–8px circular marker with a 1px `--content-primary` ring; where the data identifies a specific Ekman emotion, fill THAT marker (and optionally a `--emotion-*-secondary` wash behind that stage segment) with the matching emotion token — this is the one legitimate home of emotion tokens. An uncoded stage gets an open marker (surface fill). Valence without a named emotion: use `--content-brand` for the line and markers. The curve already ranks the stages, so do not add a second “pain point” marker to its lowest points; call out at most 1–3 pain points only when they are not the curve's own minima, as `--content-negative` text in the Opportunities lane, never as filled red badges.
...
Type sizes go UP for print distance: a poster headline is 96–128px from the type scale, body no smaller than 14px on a sheet and 20px on a poster; line lengths still ≤ 80 characters. Use the light tokens of the chosen theme for anything that will be printed (the skeleton's print block does this automatically); a dark poster is a deliberate choice with `--surface-inverse-primary` set explicitly, not the reader's OS dark mode leaking in. Charts render as static SVG (no tooltips, direct labels only), and every value the reader needs is printed on the page. Hairlines 1px; nothing lighter than `--content-disabled` on paper. No page-break rules are needed inside a one-pager because nothing may break — if content overflows, cut content, never shrink type below the floors. Export (file context): open in the browser and print to PDF with margins “None” and background graphics on; say this in one line when delivering. In the product canvas the product handles export.
...
**Required parts:** a fixed-size stage · one dominant element (a stat, a verbatim, a chart) · the lockup (or the mark alone on square stages under 600px) or the `Listen Labs /` credit line · nothing that depends on scrolling or hovering.

**Skeleton:** Standard stages — 1200×630 (Open Graph / link preview), 1080×1080 (square), 1080×1350 (portrait social), 1920×1080 (slide, presentation grid 12 / 40 / 40), 1600×900 (thumbnail). Build the stage as `.stage { width: 1200px; height: 630px; position: relative; overflow: hidden; }` with an inner safe area of 8% on every side; place on the 12-column grid inside that area. Type does NOT use the fluid `clamp()` scale — pick fixed sizes from the type scale for the stage (headline 64–96px on a 1200-wide stage, metadata 20–24px; nothing under 20px on social sizes). Big numeral + tiny label is the signature move here. Export (file context): open the file and screenshot the `.stage` element at 2× device pixel ratio (Chrome: DevTools → Capture node screenshot; or `chromium --headless --screenshot --window-size=W,H file.html`), and state the export step in one line. In the product canvas the product handles export. Provide a `data-mode="light"` pin on `<html>` so the export does not depend on the machine's OS theme.
