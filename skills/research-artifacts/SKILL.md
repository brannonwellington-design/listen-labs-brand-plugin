...
- **Density scales with width:** 12 columns → 8 → 4; gutters 24 → 16; the canonical rhythm in `skills/_shared/brand-compliance.md` steps down with it; nothing below the type floors (12px metadata, 14px body) at any width.
- Test by actually resizing, not by assuming: the composition pass in the checklist is run at all three widths.

**Canvas-safe delivery (every artifact must survive a sandboxed pane).**
- **Respond to the container, not the screen.** The artifact may render in a pane that is 500px wide on a 1600px display. Put `container-type: inline-size` on the artifact root and write the breakpoints as `@container (max-width: 600px)` … with matching `@media` queries only as a fallback for engines without container queries. Charts measure their own container (`ResizeObserver`), never `window.innerWidth`. Never use `100vh` for a layout decision inside a pane; use the container's height or intrinsic sizing (`min-height: 100vh` is acceptable only for a standalone cover page in a file context).
...
- [ ] Designed (not just reflowed) at 375 / 768 / 1280px — stacking order chosen, horizontal artifacts rotate or pane with snap points, charts re-layout per breakpoint, no horizontal page scroll, stat modules stack with separators
