...
| Ask for… | Skill |
|---|---|
| One-pager, journey map, cross-tab, concept test, persona cards, workflow diagram, dashboard, map, interactive infographic, poster or PDF page, landing page, social graphic, HTML email, or any open-ended visual | `/research-artifacts` |
...
**How it's built:**
- **Physics** (`SKILL.md`) — brand-independent laws that apply to anything, from a lone pie chart to a wall-sized journey map: one dominant element, scale contrast as hierarchy, load-bearing whitespace, responsive physics (designed at 375 / 768 / 1280, deliberate stacking, rotate-or-pane strategies for wide artifacts, charts that re-layout instead of shrinking), WCAG contrast floors, redundant encoding, typographic precision, web hygiene.
...
- **Anatomy** (`references/deliverables.md`) — a pattern library, not a menu: adopt one, compose several, or build from the physics alone. Parts, layout skeleton, phone strategy, and failure modes for fourteen familiar deliverables: one-pager, customer decision journey map, persona cross-tab explorer, concept test readout, workflow/process diagram, persona cards, chart panels & dashboards, global/geo heat maps, the longform report (handed to `/report`), web/landing pages, print pages and posters, interactive data stories, fixed-canvas social graphics, and HTML email. Plus a procedure for novel prompts.
...
- **Components** (`skills/_shared/components.md`) — the interface vocabulary every page-like output uses: buttons, links, inputs, cards, chips, tooltips, nav, footer, bands, tables, stat tiles, and empty/loading/error states, all keyed to brand tokens.
...
Regenerate manually anytime with `python3 generate_site.py`, `python3 generate_guidelines.py`, `python3 generate_brand_file.py`, and `python3 generate_skeleton_tokens.py`.
...
| Token block inside `skills/report/references/skeleton.html` and `skills/data-viz/references/skeleton.html` | `generate_skeleton_tokens.py` | `/report` and `/data-viz` skeletons (Paper + Whisp, light/dark, print-safe) |
